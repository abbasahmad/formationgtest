...
TEST(MatchersTest, ElementsAre) {
    ...
    EXPECT_THAT(v, ElementsAre(3, 4, 5, 2, 6, 7, 8, 10, 9));
}
...
TEST(MatchersTest, UnorderedElementsAre) {
    ...
    // Vérifier si un vecteur contient les mêmes éléments, indépendamment de l'ordre
    //...
}
...
TEST(MatchersTest, UnorderedElementsAreArray) {
    ...
    EXPECT_THAT(v, UnorderedElementsAreArray({ 2, 3, 4, 5, 6, 7, 8, 9 }));
...
TEST(MatchersTest, ContainerMatchers) {
    ...
    // Vérifie que chaque élément est supérieur à 1
    EXPECT_THAT(v, Each(Gt(1))); 

    // Vérifier que 6 est présent dans le vecteur
    //...

    // Vérifier que tous les éléments de u sont dans v
    //...

    // Vérifier qu'au moins un élément est supérieur à 1
    //...

    // Vérifier que chaque élément est soit supérieur à 2, soit supérieur à 0
    //...
}
...
TEST(MatchersTest, ContainerMatchers_ContainsNTimes) {
    ...
    // Vérifier que 4 apparaît exactement 2 fois
    //...

    EXPECT_THAT(v, Contains(1).Times(1));  // Vérifie que 1 n'apparaît pas
}
